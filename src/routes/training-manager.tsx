...
function TM() {
  return (
    <Shell copilot={{
      summary: "Effectiveness 72% · ROI 7×. Product-knowledge cohort needs human intervention.",
      working: ["3 of 4 cohorts on track or complete", "Avg lift +3.6 pp"],
      attention: ["Product-knowledge cohort regressed", "5 agents escalated to classroom"],
      suggestions: [
        { title: "Refresh product micro-lessons", detail: "Add rider deep-dive + 22-fund tour" },
        { title: "Pair escalations with peer coach", detail: "Anita Sharma volunteered" },
      ],
    }}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <Card title="Cohort Performance" className="xl:col-span-2">
            ...
            <tbody>
              {COHORTS.map((c) => (
                <tr key={c.name} className="border-t border-border">
                  ...
                  <td className="py-2.5"><Badge tone={c.status === "Complete" ? "green" : c.status === "Escalated" ? "mauve" : "blue"}>{c.status}</Badge></td>
                </tr>
              ))}
            </tbody>
          </table>
